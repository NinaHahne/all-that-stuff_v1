...
// §§ initialize a new game instance: // called by index.js
exports.initGame = function(sio, socket) {
    ...
    // host events:
    gameSocket.on('hostCreatesNewGame', hostCreatesNewGame);
    gameSocket.on('hostStartsGame', hostStartsGame);

    // common events:
    gameSocket.on("next turn", nextTurn);
    gameSocket.on("end game", getWinner);

    // player events:
    gameSocket.on("join a game", joinGame);
    gameSocket.on("selected piece", playerSelectedPiece);
    gameSocket.on("done building", doneBuilding);
    gameSocket.on("moving objects", movingObjects);
    gameSocket.on("made a guess", someOneGuessed);
...
// §§ HOST FUNCTIONS -----------------------------------------
function hostCreatesNewGame() {
    let socket = this;
    // Create a unique Room:
    let thisRoomId = cryptoRandomString({
        length: 5
    });
    console.log('thisRoomId: ', thisRoomId);
    // Return roomId and mySocketId to the browser client, that created the new room:
    socket.emit('newGameCreated', {
        roomId: thisRoomId,
        mySocketId: socket.id
    });
    // join the room and wait for other players:
    socket.join(thisRoomId.toString());
}

function hostStartsGame(roomId) {
    let socket = this;
    // start the game..........
}
...
function joinGame(data) {
    let socket = this;
    // Look up the roomID in the Socket.IO manager object:
    let thisRoomId = gameSocket.manager.rooms["/" + data.roomId];
    if (thisRoomId) {
        // attach the socket id to the data object:
        data.mySocketId = socket.id;
        // join the room:
        socket.join(data.roomId);
        // notify all clients in the room, that the player has joined them:
...
function nextTurn(activePlayer, activeObjects, queuedObjects) {
